//! 监听轮询主循环：负责持续轮询微信 UI、判定消息增量，并把结果投影到数据库和 sidebar，
//! 它是 runtime 主链里最核心的调度器，但只关心轮询与投影，不负责配置持久化或前端状态管理。
...
/// MonitorLoopContext 汇总监听轮询主循环所需依赖。
/// 业务上把“轮询 UI”“数据库写入”“sidebar 投影”“翻译回流”放在一轮上下文里，避免循环内频繁回到全局容器取依赖。
