...
const CHAT_DAILY_SYSTEM_PROMPT: &str = r#"你是一个微信群聊纪要助手。请基于提供的当天聊天记录，用中文输出简洁摘要。
...
待跟进事项：
3. 如果某一项没有明确信息，写“无”
4. 控制在 4-8 行内，信息密度高，不要空话"#;

const PARTICIPANT_DAILY_SYSTEM_PROMPT: &str = r#"你是一个群聊成员发言总结助手。请基于提供的当天发言记录，用中文输出简洁摘要。
...
const CHAT_OVERALL_SYSTEM_PROMPT: &str = r#"你是一个微信群聊阶段总结助手。请根据多天小结，输出这段时间内群聊的整体总结。
...
const PARTICIPANT_OVERALL_SYSTEM_PROMPT: &str = r#"你是一个群聊成员阶段发言总结助手。请根据多天小结，输出这个人在这段时间内的整体发言画像。
...
impl HistorySummaryService {
    ...
    pub async fn summarize(
        ...
        for item in &daily_transcripts {
            let summary = self
                .summarize_daily(scope.clone(), chat_name, participant, item)
                .await?;
        ...
        let overall_summary = if daily_items.len() == 1 {
            ...
            self.summarize_overall(scope.clone(), chat_name, participant, &daily_items)
                .await?
        };
