...
- `targets`：监听目标数组。
  ...
  - 运行中可以在侧边栏顶部点击“添加群”，或在左侧菜单上右键删除 target；变更会回写 `listener.json`，并平滑重启同一个 worker 生效。
...
- `tts_auto_read_active_chat`：是否自动朗读“当前选中会话”的新英文消息，默认 `true`。
  ...
  - 该开关只影响自动朗读，不影响“点击正文”或“悬停 `▶`”的手动触发。
- `width`：侧边栏宽度。
...
- 关闭“原文”且消息正文可判定为英文时，消息正文可以直接点击朗读；消息尾部也会显示一个 `▶`，鼠标移上去后同样会通过当前 `tts.provider` 朗读英文。
- 正文点击只绑定在正文字符范围，不包括时间、发送人和空白区。
- `▶` 的悬停触发带短冷却，用来压住鼠标抖动造成的重复排队。
