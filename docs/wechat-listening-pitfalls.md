...
### 25) 设置页找不到深色模式，侧边栏主题却会变
现象：
- 设置页没有“深色模式”入口，但侧边栏在系统切到深色时会自动变暗，容易误以为主题逻辑不一致或配置丢失。

根因：
- 旧实现只有样式层的 `.dark` 变量与 `prefers-color-scheme` 跟随，没有把主题策略写入 `config/listener.json`；
- `display.sidebar_appearance` 只控制透明度、模糊和文字增强，不负责浅色/深色切换。

处理：
- 主题策略统一落到 `display.theme_mode`，只允许 `light` / `dark` / `system`，默认 `system`；
- 主窗口和侧边栏共用这一个字段；设置页修改后通过既有 `settings_update -> settings-updated` 链路同步生效；
- 侧边栏启动 URL 需显式带上 `theme` 参数，避免窗口初始化时先按系统主题渲染，再被配置覆盖造成闪动。

结论：
- `display.theme_mode` 是“用什么主题”；
- `display.sidebar_appearance` 是“浮窗在该主题下怎么显示”；
- 两者职责必须分离，后续不要再把深色模式塞进 `sidebar_appearance`。

### 26) 侧边栏把占位文本当正常英文解析，自动朗读也会误触发
现象：
- 图片占位或系统提示类消息若以 `[` 开头、以 `]` 结尾，例如 `[image]`，会被侧边栏当成普通英文文本处理；
- 用户点词后会弹词义框，但这类占位文本没有实际查词价值；
- 自动朗读若遇到纯数字或纯链接消息，也会读出一串噪音。

根因：
- 侧边栏的英文分词与自动朗读之前只按“是否有文本”判断，没有额外识别占位文本、纯数字、纯链接这几类低价值消息；
- 手动朗读入口也没有放在消息头部，导致单条消息只能等自动朗读，无法按需重播。

处理：
- `SidebarView.tsx` 必须把“整条消息是 `[...]`”视为“方括号占位文本”，包括 `[image]`、`[*]`、`[]`：
  - translated 视图里直接按普通文本显示；
  - 禁止走单词分词与点击查词链路；
  - 不显示该条消息的手动朗读按钮；
- 自动朗读新增三类跳过条件：
  - 方括号占位文本；
  - 纯数字；
  - 纯链接；
- 每条消息的发送人后面补手动朗读按钮，显式点击时允许按当前显示模式重播该条消息；
- `WordPopover.tsx` 打开词义弹窗后，若词典返回美式音频，自动播放一次 `us` 发音；同一次弹窗生命周期内禁止重复自动播放。

结论：
- `[image]` 这类消息可以显示，但不能参与英文解析和自动朗读；
- 纯数字、纯链接消息保留展示，但默认不自动朗读；
- “自动朗读过滤”与“手动点按重播”是两套规则，不要混成一个总开关。
